...
2. **Reuse `@vf/llm`'s existing providers** for the narrative LLM call; web search is a _separate_ HTTP call layered on top, not a provider substitution. Keeps the Provider interface clean.
