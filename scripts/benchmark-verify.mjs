...
const renderInto = async (sub) => {
  ...
  const r = run("node", ["packages/cli/dist/index.js","preview","--cwd", projectRoot]);
  if (r.status !== 0) throw new Error(`vf preview failed: ${r.stderr}`);
  await fs.cp(path.join(projectRoot,"output","preview-faststart.mp4"),
...
for (const sub of ["r1","r2"]) {
  ...
  report.runs.push({ sub, width: p.width, height: p.height, fps: p.r_frame_rate, duration: parseFloat(p.duration) });
}
...
await fs.writeFile(".omo/evidence/task-16-verify.json", JSON.stringify(report, null, 2));
